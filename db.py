...
class DatabaseFile:
    ...
    def _extract_record_name_and_type(self, line):
        ...
        if re.search(r'^\s*record.*\(', line):
            ...
            # Get rid of the 'record'keyword and the parenthesis, and extract the record type.
            record_name = re.sub(r'\s*record.*?\(', '', line)
            # print '1', record_name
            if record_name:
                record_type = record_name.split(',')[0]
                # print 'type', record_type
            else:
                return None, None  # ill formed line
            # print '2', record_name

            # Get rid of the record type and the comma
            record_name = re.sub('^.*?,', '', record_name)
            # print '3', record_name

            # Get rid of the everything after the the trailing parenthesis. Remove double quotes as well.
            # We cannot use regular expressions here because the record name might have macros.
            pos = record_name.rfind(')')
            if pos > 0:
                record_name = record_name[0:pos]
            else:
                return None, None  # ill formed record name

            # Finally, get rid of any double quotes. In databases generated by VDCT there might not be
            # quotes surrounding the record name.
            record_name = re.sub('"', '', record_name)
            ...
            if self.filter is not None:
                ...
                return record_name.strip(), record_type.strip()
        else:
